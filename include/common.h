...
bool has_span(Bitmapset *map, int *array, int l, int n);
...
uint32 sm_add_span(sparsemap_t *map, int map_size, int span_length);

void print_bits(char *name, uint64_t value);

void bitmap_from_uint32(Bitmapset *map, uint32_t number);
void sm_bitmap_from_uint64(Bitmapset *map, int offset, uint64_t number);
uint32_t rank_uint64(uint64_t number, int n, int p);
int whats_set_uint64(uint64_t number, int bitPositions[64]);

void sm_whats_set(Bitmapset *map, int off, int len);

bool sm_is_span(Bitmapset *map, uint32 m, int len, bool value);
bool sm_occupied(Bitmapset *map, uint32 m, int len, bool value);
